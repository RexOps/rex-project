...
task "pre_check_systems",
  ...
  sub {
    ...
    my $default_tests_d = File::Spec->catdir(
        dirname( Rex::Helper::Path::get_file_path("__module__.pm") ),
        "tests.d" );
...
task "prepare",
  ...
  sub {
    ...
    $param->{start_before_deploy} //= $application->need_start_before_deploy
      // 1;
    ...
    my $rescue =
      ref $param->{rescue} ? $param->{rescue}->[0] : $param->{rescue};
